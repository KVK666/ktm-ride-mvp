    ...
    @if (loading()) {
            ...
            @for (duplicate of duplicates(); track duplicate.id) {
              <article class="ride-row">
                <div>
                  <strong>{{
                    duplicate.title ||
                      duplicate.smartTitle ||
                      dateLabel(duplicate.startedAt) + ' ride'
                  }}</strong>
      ...
      @if (viewerPhoto(); as photo) {
        <div class="media-modal" role="dialog" aria-modal="true">
          <button type="button" class="modal-close" (click)="viewerPhoto.set(null)">
      ...
      @if (slideshowOpen()) {
        <div class="media-modal slideshow" role="dialog" aria-modal="true">
          <button type="button" class="modal-close" (click)="slideshowOpen.set(false)">
            <lucide-icon name="x" size="22" />
          </button>
          @for (photo of photos(); track photo.id) {
...
export class RideDetailPageComponent implements OnDestroy, OnInit {
  ...
  readonly displayTitle = computed(() => {
    ...
    return (
      this.intelligence()?.suggestedTitle ||
      ride?.title ||
      ride?.aiTitle ||
      ride?.smartTitle ||
      `${dateLabel(ride?.startedAt)} ride`
    );
  });
  ...
  async uploadPhotos(event: Event) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const response = await this.api.request<{ photo: RideAlbumPhoto }>(
          `/rides/${encodeURIComponent(ride.id)}/photos`,
          {
            method: 'POST',
