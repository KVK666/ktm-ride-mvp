...
const { buildHomeExperience } = require("../services/journalIntelligence");
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const recentResult = await db.query(
      `${rideSelect()} where r.user_id = $1 order by r.started_at desc limit 10`,
      [req.user.id]
    );
    ...
    return res.json(buildHomeExperience({ stats, recentRides, monthRides }));
  } catch (error) {
    return next(error);
  }
});
