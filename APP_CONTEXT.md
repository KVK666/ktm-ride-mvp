...
- 2026-07-18: Fixed Saved Places and forgot-password persistence after live phone testing exposed stale legacy read/write database overrides splitting account reads from owner-scoped writes. The documented `DATABASE_URL` is now authoritative for both pools, Saved Places reads and limit checks use the primary connection, password-reset user lookup and token writes stay on the same connection, non-unique integrity failures are no longer mislabeled as duplicate place names, and mobile keeps the server-returned place immediately after a successful save.
