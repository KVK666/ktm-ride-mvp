...
- 2026-07-18: Fixed Saved Places and forgot-password persistence after live phone and DBeaver testing exposed Lightsail schema drift introduced by the manual table copy. The migrated `saved_places` and `password_reset_tokens` tables had lost their UUID and timestamp defaults, so inserts failed with null primary keys. The required defaults and account foreign keys are now repaired at startup, and the copied `users.id` is restored as a unique key. The documented `DATABASE_URL` is authoritative for both pools; Saved Places reads and limit checks use the primary connection; password-reset lookup and token writes stay on that same connection; non-unique integrity failures are no longer mislabeled as duplicate place names; and mobile keeps the server-returned place immediately after a successful save.
